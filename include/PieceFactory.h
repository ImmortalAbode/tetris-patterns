...
public:
    ...
    // Подсмотреть, какая фигура выйдет следующей (для превью в HUD), саму
    // очередь фабрики не меняя (кроме Sprint: если "мешок" пуст, придется
    // перемешать заранее - см. SprintPieceFactory - но порядок фигур от
    // этого не меняется, просто решение принимается чуть раньше).
    // Это не отдельный паттерн, а обычный дополнительный метод у уже
    // существующего Creator - иначе он не отличался бы от CreatePiece().
    virtual std::unique_ptr<Tetromino> PeekNext() = 0;
